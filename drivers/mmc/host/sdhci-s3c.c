...
#if defined(CONFIG_LINUX_BACKPORTS)
#define CFG_WIFI_SDIO_ID 2
static struct sdhci_host* sdhci_host_slot[4] = {NULL, NULL, NULL, NULL};
static int sdhci_host_id = 0;
static int sw_mci_rescan_card(unsigned insert)
{
	struct sdhci_host *host;

	BUG_ON(CFG_WIFI_SDIO_ID > 3);
	host = sdhci_host_slot[CFG_WIFI_SDIO_ID];
	if (!host)
		return -ENODEV;
...
static int __devinit sdhci_s3c_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ERR"%s id=%d\n", __func__, sdhci_host_id);
#if defined(CONFIG_LINUX_BACKPORTS)
		sdhci_host_slot[sdhci_host_id++] = host;
#endif
...
#if defined(CONFIG_MACH_MINI210) || defined(CONFIG_MACH_TINYC110)
