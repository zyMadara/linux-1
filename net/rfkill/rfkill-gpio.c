...
#define HOST_RTS_PIN S5PV210_GPA0(7)
#define HOST_CTS_PIN S5PV210_GPA0(6)
static inline void bt_bcm_rts_ctrl(int mode, int val)
{
	if (mode) {
		/* Host RTS Set to @val */
		s3c_gpio_cfgpin(HOST_RTS_PIN, S3C_GPIO_OUTPUT);
		s3c_gpio_setpull(HOST_RTS_PIN, S3C_GPIO_PULL_UP);
		gpio_set_value(HOST_RTS_PIN, val);
	} else {
		/* restore BT RTS state */
		s3c_gpio_cfgpin(HOST_RTS_PIN, S3C_GPIO_SFN(2));
	}
}
...
static int rfkill_gpio_set_power(void *data, bool blocked)
{
	...
	if (blocked) {
		...
		for (i = 0; i < 30; i++) {
			msleep(1);
			if (!(cts = gpio_get_value(HOST_CTS_PIN)))
				break;
		}
		pr_info("rfkill-gpio: wake peer by RTS, %s\n", cts ? "failed" : "ok");
		bt_bcm_rts_ctrl(0, 0);
	}
